...
export default function TestimonialCard({ text, name, field }: CardProps) {
  return (
    <div className="w-full justify-between flex flex-col gap-2.5 rounded-xl bg-[#0f0f0f] border border-[#1c1c1c]">
      <div className="flex flex-col p-4 gap-4">
        ...
        <p className="text-[15px] text-[#ababab]">{text}</p>
      </div>
      <div className="bg-[#171717] py-2 px-4 flex items-center gap-3">
        ...
        <div>
          <p className="font-medium text-sm text-[#ededed]">{name}</p>
          <p className="text-[13px] text-[#ababab]">{field}</p>
        </div>
      </div>
    </div>
  );
